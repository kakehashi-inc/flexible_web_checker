...
class UrlListViewTests(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(username='testuser@example.com', email='testuser@example.com', password='password', nickname='Test User')
        self.user.is_active = True  # Activate the user for login
        self.user.save()
        login_successful = self.client.login(username='testuser@example.com', password='password')
        self.assertTrue(login_successful, "Test client login failed") # Add assertion to check login success
        
        self.url_categorized = UrlItem.objects.create(user=self.user, url='https://example.com/cat', title='Categorized')
        
        self.url_uncategorized = UrlItem.objects.create(user=self.user, url='https://example.com/uncat', title='Uncategorized')
    ...
    def test_uncategorized_tab_filter_logic(self):
        ...
        response_uncat = self.client.get(reverse('url_manager:url_list') + '?tab=uncategorized')
