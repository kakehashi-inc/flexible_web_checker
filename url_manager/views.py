...
def url_list(request):
    """URL一覧ビュー"""
    url_items = UrlItem.objects.filter(
        user=request.user
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            for url_str in urls_to_add:
                ...
                if UrlItem.objects.filter(
                    user=request.user, url=url_str
                ...
                try:
                    ...
                    if not title:
                        ...
                        messages.warning(
                            request,
                            _(
                                'URL "{url}" のタイトルを取得できませんでした。URLをタイトルとして使用します。'
                            ).format(url=url_str),
                        )
            ...
            if added_count > 0:
                messages.success(
                    request,
                    _("{count}件のURLを追加しました。").format(count=added_count),
                )
            if error_urls:
                error_html = (
                    _("以下のURLは追加できませんでした:")
                    + '<ul class="list-disc list-inside">'
                    + "".join([f"<li>{err}</li>" for err in error_urls])
                    + "</ul>"
                )
