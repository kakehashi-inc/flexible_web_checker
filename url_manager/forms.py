...
class UrlItemForm(forms.ModelForm):
    ...
    class Meta:
        model = UrlItem
        fields = ('url', 'title', 'check_type', 'html_selector', 'html_custom_condition')
        widgets = {
            'html_selector': forms.Textarea(attrs={'rows': 4, 'placeholder': _('例: div.content\n#main-content h2')}),
        }
    ...
    def clean_html_selector(self):
        ...
        if check_type == 'HTML_CUSTOM' and not html_selector:
            raise forms.ValidationError(_('HTMLカスタムチェックタイプを選択した場合は、HTMLセレクタを入力してください。'))
