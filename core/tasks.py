    ...
    try:
        ...
        try:
            if url_item.check_type == 'RSS':
                is_updated, new_hash = check_rss_feed(url_item.url, url_item.last_content_hash)
            elif url_item.check_type == 'HTML_STANDARD':
                is_updated, new_hash = check_html_standard(url_item.url, url_item.last_content_hash)
            elif url_item.check_type == 'HTML_CUSTOM':
                selectors = url_item.html_selector.splitlines() if url_item.html_selector else []
                condition = url_item.html_custom_condition or 'OR'
                is_updated, new_hash = check_html_custom(url_item.url, selectors, condition, url_item.last_content_hash)
...
def update_thumbnail(url_item_id):
    ...
    try:
        ...
        logger.error(f"Unexpected error updating thumbnail for URL {url_item_id}: {str(e)}")
        return False
...
def check_all_urls_task(user_id):
    ...
    try:
        ...
        logger.info(f"Scheduled update checks for {url_items.count()} URLs for user ID: {user_id}")
        return True
